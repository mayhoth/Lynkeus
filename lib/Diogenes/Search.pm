...
# Lynkeus
sub get_hits {
  croak "get_hits is an instance method: $!" unless ref $_[0];
  $_[0]->{hits};
}
...
sub pgrep 
{
	...
	my $author_iteration = 0;
		...
		if ($self->{lynkeus_fh} and $final_pass) {
		  ...
		  # print $ARGV, " = ", $auth_num, " => ", pos $buf, "\n";
		  # print "match_start: $-[0]\n";
		  # print substr ($buf, $-[0], ($+[0] - $-[0])), "\n";
...
sub latin_pattern
{
    ...
    $pat =~ 
        s/(\[[^\]]*\][*+?]?)(?!\s+$)/$1(?:-\[$non_alpha_with_space\]\*|\[$non_alpha\]\*)/g;
    # spaces at the start + some lookbehind
    # the first line seems better suited, as it precludes cases where
    # the ()/\ is a modifier for the following word. But is it safe?
    $pat =~ s/^\s+/(?<![A-Za-z][()]?[\\\/]?)(?<!\-[\\x80-\\xff])(?<!\-[\\x80-\\xff][\\x80-\\xff])/g;
    # $pat =~ s/^\s+/(?<![()\\\/A-Za-z])(?<!\-[\\x80-\\xff])(?<!\-[\\x80-\\xff][\\x80-\\xff])/g; 
    $pat =~ s/\s+$/(?![^- A-Za-z]*[-A-Za-z])/g;                     # spaces at the end
    $pat =~ s/(?<!^)(?<!-)\s+/\\s+\[$non_alpha_with_space\]\*/g;  # other spaces (not at start or end)
    $pat =~ s/\((?!\?)/(?:/g;                          # turn ( into (?: for speed
...
sub extract_hits 
{
    ...
    HIT: foreach $match ( 0 .. $#{ $self->{seen}{$auth} } ) 
    {
        ...
        print STDERR "Buf_start: $buf_start\n" if $self->{debug};
...
#            print STDERR  "$seens\n";
	...
	# Lynkeus ($info is defined only when processing the final
	# results: Here, we do not want duplicate hits!
	last if $self->{bib_info_printed}{$self->{auth_num}}{$self->{work_num}} and $info;
