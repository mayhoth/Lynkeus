...
sub bench_indexed {
  ...
  my @wlist;
  for my $form (@forms) {
    my ($ref, @words) = $query->read_index(uc($form));
    push @wlist, \@words;
  }
...
unless ($alt) {
  ...
  cmpthese
    ($count,
     {
      $alt => sub { bench($alt, $stem) },
      # 'ind' => sub { bench_indexed }
     }
    );
}
